...
import type { AgentView, GraphState, MessageView, ToolView } from "../../src/reducer";
import type { MessageChannel, MessageStatus, TelemetryEvent } from "../../shared/protocol";
import { eventLabel, livePresence, liveStream, useDashboard, useFilteredGraph, useTimelineView, type Filters } from "./state";

export type EntityId = { type: "instance" | "agent" | "peer" | "tool"; key: string };
type Point = { x: number; y: number };
type Rect = Point & { width: number; height: number; entity: EntityId; color: string; label: string; status: string; meta?: string; detail?: string; span?: string; contextPercent?: number };
...
export function toolOwnerKey(tool: { agentKey: string; producerId: string; sessionId: string }, agentKeys: ReadonlySet<string>): string {
  return agentKeys.has(tool.agentKey) ? tool.agentKey : `${tool.producerId}::${tool.sessionId}`;
}
...
function placeToolChips(owner: Rect, tools: readonly ToolView[], rects: Rect[]): void {
  ...
  visible.forEach((tool, index) => {
    rects.push({
      ...
      entity: { type: "tool", key: tool.key }, color: toolColor(tool.status), label: tool.name, status: tool.status,
    });
  });
}
...
export function computeLayout(graph: GraphState, width: number, height: number): Layout {
  ...
  instances.forEach(([sessionId, instance], index) => {
    ...
    for (const tool of Object.values(graph.tools)) {
      if (`${tool.producerId}::${tool.sessionId}` !== sessionId) continue;
    ...
    const root: Rect = {
      ...
      entity: { type: "instance", key: sessionId }, color: statusColor(instance.status),
  ...
  useEffect(() => {
    ...
    let raf = 0; const draw = (): void => {
      ...
      instances.forEach((root) => {
        ...
        const owned = current.rects.filter((rect) => rect.entity.type !== "peer" && rect.x >= cardX && rect.x + rect.width <= cardX + cardW + 1 && rect.y >= cardY);
        const bottom = Math.max(root.y + root.height, ...owned.map((rect) => rect.y + rect.height));
        const cardH = Math.min(current.height - cardY - 22, Math.max(root.height + 178, bottom - cardY + 18));
...
function Inspector({ graph, selected, onClose, onSelect }: { graph: GraphState; selected?: EntityId; onClose: () => void; onSelect: (entity: EntityId) => void }): ReactElement {
  ...
  const toolInstanceKey = tool ? `${tool.producerId}::${tool.sessionId}` : "";
  ...
  return <aside className="inspector" aria-label="Selected node inspector">
    ...
    {selected ? <>
      ...
      {instance && <><div className="context-meter"><div><span>CONTEXT WINDOW</span><b>{instance.contextPercent}%</b></div><div className="meter"><i style={{ width: `${Math.min(100, instance.contextPercent)}%` }} /></div></div><dl className="detail-list"><div><dt>PERSONA</dt><dd>{instance.persona || "unassigned"}</dd></div><div><dt>MODEL</dt><dd>{instance.model || "auto"}</dd></div><div><dt>DURATION</dt><dd>{formatElapsed(instance.updatedAt - instance.startedAt)}</dd></div><div><dt>PID</dt><dd>{instance.pid || "—"}</dd></div></dl></>}
      {agent && <dl className="detail-list"><div><dt>KIND</dt><dd>{agent.kind}</dd></div><div><dt>AGENT</dt><dd>{agent.agent ?? "—"}</dd></div><div><dt>PERSONA</dt><dd>{agent.persona ?? "—"}</dd></div><div><dt>MODEL</dt><dd>{modelLabel(agent.model) || "—"}</dd></div><div><dt>DURATION</dt><dd>{formatElapsed((agent.endedAt ?? agent.updatedAt) - agent.startedAt)}</dd></div><div><dt>TOOLS</dt><dd>{(() => { const stats = toolStats(relatedTools); return `${stats.total}${stats.failed ? ` · ${stats.failed} failed` : ""}${stats.running ? ` · ${stats.running} live` : ""}`; })()}</dd></div></dl>}
      ...
      <div className="inspector-section"><span className="eyebrow">RECENT TRAFFIC</span>{relatedMessages.length ? relatedMessages.map((message) => <div className="traffic-row" key={message.key}><span className={`channel-mark ${message.channel}`} /><span>{message.kind}</span><b>{message.status}</b></div>) : <div className="empty-small">No message frames for this node.</div>}</div>
...
function App(): ReactElement {
  ...
  const attentionCount = [...Object.values(presenceGraph.instances), ...Object.values(presenceGraph.agents)].filter(isAttention).length;
  ...
  const runningTools = Object.values(presenceGraph.tools).filter((tool) => tool.status === "running" && liveStream(presenceGraph.instances[`${tool.producerId}::${tool.sessionId}`]?.status ?? "stale")).length;
